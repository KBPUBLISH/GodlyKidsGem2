...
Round-robin uses only regions where the model is available:

- **Global:** `global` (use `VERTEX_AI_IMAGE_LOCATION=global`)
...
Asia, Australia, Middle East, etc. are not supported for this model.
